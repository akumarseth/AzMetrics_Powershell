...
foreach ($ResourceGroup in $ResourceGroups)
{
    ...
    #$VMs = az vm list -ResourceGroupName $ResourceGroup
    ...
    foreach ($vm in $VMs)
    {
        ...
        $MetricsDefinitions = Get-AzMetricDefinition -ResourceId $Vm.Id
        $MetricsDefinitions
            ...
            #{
                ...
                foreach($aggtype in $metric.SupportedAggregationTypes) 
                {
                    ...
                    if ($aggtype -ne "None") 
                    {
                        ...
                        $MetricsDetails = Get-AzMetric -ResourceId $Vm.Id -MetricName $metric.Name.Value -TimeGrain 00:15:00 -StartTime (Get-Date).AddDays(-1) -EndTime (Get-Date) -AggregationType $aggtype -WarningAction SilentlyContinue                    
    ...
    $DataToExport | Export-Csv -Path C:\AZMetric_export\vm_$ResourceGroup.csv
    $DataToExport | Export-Excel -Path C:\AZMetric_export\vm_$ResourceGroup.xlsx
